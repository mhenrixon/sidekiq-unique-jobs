...
RSpec.describe SidekiqUniqueJobs::Script, redis: :redis do
  ...
  describe ".call" do
    subject(:call) { described_class::Caller.call_script(script_name, keys, argv) }
    ...
    context "when conn.evalsha raises Redis::CommandError" do
      before do
        call_count = 0
        allow(described_class).to receive(:execute_script).with(script_name, anything, keys, argv) do
          call_count += 1
      ...
      specify do
        ...
        expect(described_class).to have_received(:execute_script).with(script_name, anything, keys, argv).once
      end
      ...
      context "when error message is No matching script" do
        ...
        specify do
          ...
          expect(described_class).to have_received(:execute_script).with(script_name, anything, keys, argv).twice
        end
      end
    end
  end
