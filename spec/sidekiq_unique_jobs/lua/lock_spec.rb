...
RSpec.describe "lock.lua" do
  ...
  context "when lock_type == :until_expired" do
    ...
    it "expires the correct keys" do
      expect { lock }.to change { zcard(key.changelog) }.by(1)

      expect(ttl(key.digest)).to eq(lock_ttl / 1000)
      expect(ttl(key.locked)).to eq(lock_ttl / 1000)
    end
  end
