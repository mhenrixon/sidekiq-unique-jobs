...
RSpec.describe SidekiqUniqueJobs::Lock::UntilAndWhileExecuting do
  ...
  describe "#execute" do
    ...
    before do
      allow(lock).to receive(:unlock).and_return(unlock_return_value)
      allow(lock).to receive(:runtime_lock).and_return(runtime_lock)
      allow(runtime_lock).to receive(:execute).and_yield
    end

    context "when unlock returns true" do
      let(:unlock_return_value) { true }
    ...
    context "when unlock returns false" do
      let(:unlock_return_value) { false }
