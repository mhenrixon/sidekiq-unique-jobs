...
redis.log(redis.LOG_DEBUG, "old_lock.lua - Check for: " .. job_id .. " if can lock")

local stored_jid = redis.call('GET', exists_key)

if not stored_jid or stored_jid == false then
  redis.log(redis.LOG_DEBUG, "old_lock.lua - No existing locks for: " .. exists_key)
elseif stored_jid == job_id then
  redis.log(redis.LOG_DEBUG, "old_lock.lua - Lock owned by current job_id: " .. job_id)
else
  redis.log(redis.LOG_DEBUG, "old_lock.lua - Lock owned by another job_id: " .. stored_jid)
  return stored_jid
end

----------------------------------------------------------------
-- TODO: Legacy support (Remove in v6.1)
redis.log(redis.LOG_DEBUG, "old_lock.lua - Check for legacy lock")

local old_jid    = redis.call('GET', unique_digest)

if not old_jid or old_jid == false then
  redis.log(redis.LOG_DEBUG, "old_lock.lua - No existing legacy lock for:" .. exists_key)
elseif old_jid == job_id or old_jid == '2'  then
  redis.log(redis.LOG_DEBUG, "old_lock.lua - Converting legacy lock for current job_id: " .. job_id)
  redis.call('DEL', unique_digest)
else
  redis.log(redis.LOG_DEBUG, "old_lock.lua - Existing legacy lock for another job_id: " .. old_jid)
  return old_jid
end

redis.log(redis.LOG_DEBUG, "old_lock.lua - Creating necessary lock keys")
...
if lock_type == "until_expired" and ttl then
  redis.log(redis.LOG_DEBUG, "old_lock.lua - Until expired job. Setting ttl on all keys")
